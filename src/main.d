...
int main()
{
    ...
    int returnCode = al_run_allegro(() { return theExample(); });
...
int theExample()
{
    ...
    if (!display) {
        writeln("Can't create display.");
        return -1;
    }
    writeln("Display pointer is ", cast (ulong) display);
    ...
    while (1) {
        ...
        if (event.type == ALLEGRO_EVENT_MOUSE_WARPED) {
            writeln("Warp to x=", event.mouse.x,
                " y=", event.mouse.y,
                " z=", event.mouse.z,
                " w=", event.mouse.w,
                " dx=", event.mouse.dx,
                " dy=", event.mouse.dy,
                " d=", cast (ulong) event.mouse.display);
        }
